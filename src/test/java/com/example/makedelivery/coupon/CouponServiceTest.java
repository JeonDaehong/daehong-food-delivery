...
public class CouponServiceTest {
    ...
    @Test
    @DisplayName("한 명당 한 개의 쿠폰만 발급 가능합니다.")
    public void applyCouponMultiThreadTestRedisOneUser() throws InterruptedException {
        ...
        for ( int i=0; i<THREAD_COUNT; i++ ) {
            executorService.submit(() -> {
                try {
                    couponService.applyCoupon(1L, Coupon.CouponType.NEW_YEAR_EVENT_POINT_5000, Coupon.Capacity.NEW_YEAR_EVENT_POINT_5000);
                } finally {
                    countDownLatch.countDown();
                }
            });
